...
void inicializa() {
    ...
    glEnable(GL_LIGHT0);  // Ativa a luz 0 (padrão)

    // Configura a posição da luz (como um "sol" vindo de cima, atrás e um pouco à direita)
    GLfloat light_pos[] = { 0.0f, 5.0f, -5.0f, 1.0f };  // Luz acima e atrás da câmera, deslocada para a direita
    glLightfv(GL_LIGHT0, GL_POSITION, light_pos);  // Definir a posição da luz

    // Configura intensidade da luz difusa (iluminação principal)
    GLfloat lightIntensity = 0.8f;  // Intensidade da luz difusa
    glLightf(GL_LIGHT0, GL_DIFFUSE, lightIntensity);  // Definir a intensidade da luz

    // Configura propriedades da luz ambiente (iluminação suave de fundo)
    GLfloat luzAmbiente[] = { 0.1f, 0.1f, 0.1f, 1.0f };  // Luz ambiente suave
    glLightfv(GL_LIGHT0, GL_AMBIENT, luzAmbiente);  // Definir luz ambiente

    // Propriedades do material
    GLfloat materialSpecular[] = { 0.3f, 0.3f, 0.3f, 1.0f };  // Reflexão mais suave
    GLfloat materialShininess[] = { 10.0f };  // Brilho suave (menor brilho)
    glMaterialfv(GL_FRONT, GL_SPECULAR, materialSpecular);  // Definir reflexão especular
    glMaterialfv(GL_FRONT, GL_SHININESS, materialShininess);  // Definir brilho do material
}
